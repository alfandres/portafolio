import './ReadMoreShopi.css'

function ReadMoreShopi() {

    return(
        <>
            <p className='heading--read-more-shopi'>
                Shopi
            </p>
            <p className='para--read-more-shopi'>
                Shopi es tu tienda en línea completa, donde puedes explorar y comprar una amplia variedad de productos.<br/>

                <br/>Con el proyecto Shopi pude aprender sobre; el manejo y consumo de una api, la instalación y uso de tailwindcss, el manejo de enrutamiento con React Router, la instalación de vite, y JavaScript XML.<br/>

                <br/>El objetivo al hacer este proyecto era aprender a consumir una api, en este caso utilicé Fake Store API.
            </p>
        </>
    );
}

export default ReadMoreShopi;